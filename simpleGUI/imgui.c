...
gs_menu_color = {
	"PunkBlue",      "PunkBlue",	"PunkPink",	    "PunkPink",   0 , // 不填充
},
...
static int menuItem(int id, double x, double y, double w, double h, char *label)
{
	...
	mySetPenColor(frameColor);
	drawMenuBox(x, y, w, h, gs_menu_color.fillflag, label, 'L', labelColor);
...
void drawBox(double x, double y, double w, double h, int fillflag, char *label, char labelAlignment, char *labelColor)
{
	...
	if( label && strlen(label)>0 ) {
		...
		if( labelAlignment=='L' )
			...
			MovePen(x+w-fa/2-TextStringWidth(label)-0.25, y+h/2-fa/2);
		else // if( labelAlignment=='C'
			MovePen(x+(w-TextStringWidth(label))/2-0.25, y+h/2-fa/2);
	    SetPointSize(25);
		DrawTextString(label);
        SetPointSize(2);
	}
}
void drawMenuBox(double x, double y, double w, double h, int fillflag, char *label, char labelAlignment, char *labelColor)
{
	double fa = GetFontAscent();
	// rect
	drawRectangle(x,y,w,h,fillflag);
